...
network = ToyNet(model_id, project_dir="/root/ebms_proposals/ebm_age").cuda()
...
for model_i in range(M):
    network.load_state_dict(torch.load("/root/ebms_proposals/ebm_age/training_logs/model_%s_%d/checkpoints/model_%s_epoch_%d.pth" % (model_id, model_i, model_id, epoch)))
    ...
    for step, (xs, ys) in enumerate(test_loader):
        with torch.no_grad():
            ...
            y_samples = np.linspace(1.0, 60.0, num_samples) # (shape: (num_samples, ))
