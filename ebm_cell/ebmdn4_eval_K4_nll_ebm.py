...
for model_i in range(M):
    ...
    for step, (xs, ys) in enumerate(test_loader):
        with torch.no_grad():
            ...
            y_samples = np.linspace(1.0, 200.0, num_samples) # (shape: (num_samples, ))
            ...
            log_p_ys = scores_gt - torch.logsumexp(scores, dim=1) - math.log(199) + math.log(num_samples) # (shape: (batch_size))
