...
network = ToyNet(model_id, project_dir="/root/project5/utkface64").cuda()
...
for model_i in range(M):
    network.load_state_dict(torch.load("/root/project5/utkface64/training_logs/model_%s_%d/checkpoints/mdn_model_%s_epoch_%d.pth" % (model_id, model_i, model_id, epoch)))
