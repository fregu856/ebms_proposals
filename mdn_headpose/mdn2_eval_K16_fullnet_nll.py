...
network = ToyNet(model_id, project_dir="/root/project5/headpose").cuda()
...
for model_i in range(M):
    network.load_state_dict(torch.load("/root/project5/headpose/training_logs/model_%s_%d/checkpoints/model_%s_epoch_%d.pth" % (model_id, model_i, model_id, epoch)))
