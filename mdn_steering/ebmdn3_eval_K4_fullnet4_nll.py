...
model_id = "ebmdn3_train_K4_fullnet4"
M = 20

network = ToyNet(model_id, project_dir="/root/ebms_proposals/mdn_steering").cuda()
...
for model_i in range(M):
    network.load_state_dict(torch.load("/root/ebms_proposals/mdn_steering/training_logs/model_%s_%d/checkpoints/mdn_model_%s_epoch_%d.pth" % (model_id, model_i, model_id, epoch)))
